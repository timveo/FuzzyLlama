...
export class AgentsController {
  ...
  async execute(@Body() executeDto: ExecuteAgentDto, @CurrentUser() user: RequestUser) {
  ...
  @ApiResponse({
    status: 200,
    description: 'Agent execution started, results streaming via WebSocket',
  })
  @ApiResponse({ status: 400, description: 'Execution limit reached or invalid request' })
  @ApiResponse({ status: 403, description: 'Cannot execute agent for project you do not own' })
  async executeStream(@Body() executeDto: ExecuteAgentDto, @CurrentUser() user: RequestUser) {
    // Start agent execution with streaming
    const agentExecutionId = await this.executionService.executeAgentStream(executeDto, user.id, {
  ...
  async getHistory(@Query('projectId') projectId: string, @CurrentUser() user: RequestUser) {
  ...
  async getProgress(@Param('projectId') projectId: string, @CurrentUser() _user: RequestUser) {
  ...
  async getNextTask(@Param('projectId') projectId: string, @CurrentUser() _user: RequestUser) {
  ...
  async routeTask(@Param('projectId') projectId: string, @CurrentUser() user: RequestUser) {
  ...
  async executeNext(@Param('projectId') projectId: string, @CurrentUser() user: RequestUser) {
  ...
  async getWorkflowStatus(@Param('projectId') projectId: string, @CurrentUser() user: RequestUser) {
  ...
  async submitIntake(
    ...
    return this.workflowCoordinator.submitIntakeAnswers(body.projectId, user.id, body.answers);
  }
